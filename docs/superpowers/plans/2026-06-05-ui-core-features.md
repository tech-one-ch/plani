...
export async function requireProjectAccess(projectId: string) {
  ...
  const project = await db
    .select({ id: projects.id, workspaceId: projects.workspaceId, name: projects.name, slug: projects.slug, color: projects.color })
    .from(projects)
...
export async function Topbar() {
  ...
  const activeWorkspace = userWorkspaces.find((w) => {
  ...
  return (
    <header className="flex h-10 items-center justify-between border-b border-border-subtle bg-bg-sidebar px-4 flex-shrink-0">
      <div className="flex items-center gap-3">
        <Link href="/dashboard" className="text-sm font-bold text-text-white tracking-tight">
          plani
        </Link>
        {activeWorkspace && (
          <span className="rounded border border-border-default bg-bg-elevated px-2 py-0.5 text-xs text-text-secondary">
            {activeWorkspace.name}
          </span>
        )}
      </div>
      <div className="flex items-center gap-3">
        <Link href="/settings" className="text-text-muted hover:text-text-secondary transition-colors">
          <Settings size={15} />
        </Link>
        <div
          className="flex h-6 w-6 items-center justify-center rounded-full bg-accent-subtle text-xs font-bold text-accent"
...
export function Sidebar({ workspace, projects }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "flex flex-shrink-0 flex-col border-r border-border-subtle bg-bg-sidebar transition-all duration-200",
        collapsed ? "w-12" : "w-56",
      )}
    >
      {/* Navigation */}
      <nav className="flex flex-col gap-0.5 p-2 pt-3">
        {!collapsed && (
          <p className="mb-1 px-2 text-[10px] uppercase tracking-widest text-text-muted">
            Navigation
          </p>
        )}
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto p-2">
        ...
        <Link
          href="/dashboard"
          className={cn(
            "flex items-center gap-2.5 rounded-md px-2 py-1.5 text-xs text-text-muted transition-colors hover:text-text-secondary",
            collapsed && "justify-center",
          )}
      ...
      <button
        onClick={toggle}
        className="flex items-center justify-center border-t border-border-subtle p-2.5 text-text-muted transition-colors hover:text-text-secondary"
...
export default function NewWorkspacePage() {
  ...
  return (
    <div className="flex h-full items-center justify-center">
      <div className="w-full max-w-sm rounded-lg border border-border-subtle bg-bg-elevated p-8">
        <h1 className="mb-1 text-lg font-semibold text-text-white">Créer un workspace</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              required
              className="w-full rounded-md border border-border-default bg-bg-app px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-accent focus:outline-none"
            />
          </div>
          <button
            type="submit"
...
export function CreateProjectModal({ workspaceId, onClose }: CreateProjectModalProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm rounded-lg border border-border-subtle bg-bg-elevated p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-5 flex items-center justify-between">
          <h2 className="font-semibold text-text-white">Nouveau projet</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-text-secondary">Couleur</label>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="rounded-md px-4 py-1.5 text-sm text-text-secondary hover:text-text-primary"
            >
              Annuler
            </button>
            <button
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-text-white">Projets</h1>
        <button
      ...
      {projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border-default py-20 text-center">
          <p className="mb-3 text-sm text-text-secondary">Aucun projet pour l'instant.</p>
          <button
            onClick={() => setShowModal(true)}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {projects.map((project) => (
            <Link
              key={project.id}
              href={`/projects/${project.id}/board`}
              className="group rounded-lg border border-border-subtle bg-bg-elevated p-5 transition-colors hover:border-border-default"
            >
              <div className="mb-3 flex items-center gap-2.5">
                <span
                  ...
                  style={{ backgroundColor: project.color }}
                />
                <span className="font-medium text-text-primary group-hover:text-text-white transition-colors">
                  {project.name}
                </span>
              </div>
...
export default async function ProjectLayout({ children, params }: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-11 flex-shrink-0 items-center border-b border-border-subtle bg-bg-app px-6 gap-1">
        <span className="mr-4 text-sm font-semibold text-text-primary">{project.name}</span>
        {TABS.map((tab) => (
          tab.disabled ? (
            <span
              key={tab.slug}
              className="cursor-not-allowed px-3 py-2 text-xs text-text-muted"
            ...
            <Link
              ...
              className="px-3 py-2 text-xs text-text-secondary hover:text-text-primary transition-colors border-b-2 border-transparent data-[active=true]:border-accent data-[active=true]:text-text-white"
            >
              {tab.label}
            </Link>
          )
...
export default async function SettingsPage() {
  ...
  return (
    <div className="p-8 max-w-2xl">
      <h1 className="mb-6 text-xl font-semibold text-text-white">Paramètres du workspace</h1>

      <div className="mb-6 rounded-lg border border-border-subtle bg-bg-elevated p-5">
        <h2 className="mb-4 text-sm font-medium text-text-primary">Informations générales</h2>
        <div className="flex flex-col gap-3">
          <div>
            <p className="mb-0.5 text-xs text-text-secondary">Nom</p>
            <p className="text-sm text-text-primary">{workspace.name}</p>
          </div>
          <div>
            <p className="mb-0.5 text-xs text-text-secondary">Slug</p>
            <p className="text-sm font-mono text-text-secondary">{workspace.slug}</p>
          </div>
        </div>
      </div>

      <div className="rounded-lg border border-border-subtle bg-bg-elevated p-5">
        <h2 className="mb-3 text-sm font-medium text-text-primary">Membres</h2>
        <Link
...
async function getTaskWithAccess(taskId: string, userId: string) {
  ...
  if (!task) return { task: null, error: NextResponse.json({ error: "Not Found" }, { status: 404 }) };
  ...
  if (!project) return { task: null, error: NextResponse.json({ error: "Not Found" }, { status: 404 }) };
  ...
  if (!member) return { task: null, error: NextResponse.json({ error: "Forbidden" }, { status: 403 }) };
...
const STATUS_LABELS = { backlog: "Backlog", todo: "À faire", in_progress: "En cours", done: "Terminé" };
const PRIORITY_LABELS = { low: "Basse", medium: "Moyenne", high: "Haute" };
const PRIORITY_COLORS = { low: "text-priority-low", medium: "text-priority-medium", high: "text-priority-high" };
...
export function TaskDetailPanel({ task, onClose, onUpdate, onDelete }: Props) {
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="fixed right-0 top-0 z-50 flex h-full w-96 flex-col border-l border-border-subtle bg-bg-elevated shadow-xl"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border-subtle px-5 py-3">
          <span className="text-xs text-text-muted">Détail de la tâche</span>
          <button onClick={onClose} className="text-text-muted hover:text-text-secondary">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <input
            ...
            className="mb-4 w-full bg-transparent text-base font-medium text-text-white focus:outline-none"
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <span className="w-20 text-xs text-text-muted">Statut</span>
              <select
                value={status}
                onChange={(e) => {
                ...
                className="rounded border border-border-default bg-bg-app px-2 py-1 text-xs text-text-primary focus:outline-none"
              >
            ...
            <div className="flex items-center gap-3">
              <span className="w-20 text-xs text-text-muted">Priorité</span>
              <select
                value={priority}
                onChange={(e) => {
            ...
            <div className="flex items-center gap-3">
              <span className="w-20 text-xs text-text-muted">Échéance</span>
              <input
                type="date"
                value={dueDate}
                onChange={(e) => {
          ...
          <div className="mt-5">
            ...
            <textarea
              ...
              className="w-full resize-none rounded border border-border-default bg-bg-app p-3 text-sm text-text-primary placeholder:text-text-muted focus:border-border-default focus:outline-none"
            />
          </div>
        </div>
        ...
        <div className="border-t border-border-subtle px-5 py-3">
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-priority-high hover:underline"
          >
            <Trash2 size={13} />
            Supprimer la tâche
          </button>
...
export function TaskCard({ task, onClick }: Props) {
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={cn(
        "cursor-pointer rounded-md border border-border-subtle bg-bg-elevated p-3 transition-colors hover:border-border-default",
        isDragging && "opacity-40",
      )}
    >
      <p className="mb-2 line-clamp-2 text-xs text-text-primary">{task.title}</p>
      <div className="flex items-center gap-2">
        <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-medium", PRIORITY_STYLE[task.priority])}>
          {PRIORITY_LABEL[task.priority]}
        </span>
        {task.dueDate && (
          <span className={cn("ml-auto text-[10px]", isOverdue ? "text-priority-high" : "text-text-muted")}>
  ...
  return (
    <div className="flex w-60 flex-shrink-0 flex-col gap-2">
      ...
      <div className="flex items-center justify-between px-1">
        ...
        <span className="rounded bg-bg-elevated px-1.5 text-[10px] text-text-muted">{tasks.length}</span>
      </div>
      ...
      >
        ...
        <input
          ...
          className="rounded border border-dashed border-border-default bg-transparent px-2 py-1.5 text-xs text-text-muted placeholder:text-text-muted focus:border-border-subtle focus:outline-none focus:text-text-primary"
        />
      </div>
    </div>
  );
...
const STATUS_LABEL = { backlog: "Backlog", todo: "À faire", in_progress: "En cours", done: "Terminé" };
const PRIORITY_LABEL = { low: "Basse", medium: "Moyenne", high: "Haute" };
const PRIORITY_DOT = { high: "bg-priority-high", medium: "bg-priority-medium", low: "bg-priority-low" };
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border-subtle bg-bg-elevated">
                <th className="px-4 py-2.5 text-left text-xs font-medium text-text-muted">Titre</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-text-muted">Statut</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-text-muted">Priorité</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-text-muted">Échéance</th>
              </tr>
            </thead>
            <tbody>
              {tasks.map((task) => (
                <tr
                  ...
                  className="cursor-pointer border-b border-border-subtle bg-bg-app transition-colors hover:bg-bg-hover"
                >
                  <td className="px-4 py-2.5 text-xs text-text-primary">{task.title}</td>
                  <td className="px-4 py-2.5 text-xs text-text-secondary">{STATUS_LABEL[task.status]}</td>
...
async function getNoteWithAccess(noteId: string, userId: string) {
  const db = getDb();
  const note = await db.select().from(notes).where(eq(notes.id, noteId)).limit(1).then((r) => r[0]);
  if (!note) return { note: null, error: NextResponse.json({ error: "Not Found" }, { status: 404 }) };

  const project = await db.select().from(projects).where(eq(projects.id, note.projectId)).limit(1).then((r) => r[0]);
  if (!project) return { note: null, error: NextResponse.json({ error: "Not Found" }, { status: 404 }) };
  ...
  if (!member) return { note: null, error: NextResponse.json({ error: "Forbidden" }, { status: 403 }) };
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const updated = await db.update(notes).set(updates).where(eq(notes.id, noteId)).returning().then((r) => r[0]);
  return NextResponse.json(updated);
}
...
export function NoteList({ notes, selectedId, onSelect, onNew }: Props) {
  function excerpt(content: string) {
    return content.replace(/<[^>]+>/g, " ").trim().substring(0, 60) || "Aucun contenu";
  }

  return (
    <div className="flex h-full w-60 flex-shrink-0 flex-col border-r border-border-subtle bg-bg-sidebar">
      <div className="flex items-center justify-between border-b border-border-subtle px-4 py-3">
        <span className="text-xs font-medium text-text-secondary">Notes</span>
        <button onClick={onNew} className="text-text-muted hover:text-text-secondary transition-colors">
          <Plus size={14} />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto">
        {notes.length === 0 && (
          <p className="p-4 text-xs text-text-muted">Aucune note. Créez-en une.</p>
        )}
        {notes.map((note) => (
          <button
            key={note.id}
            ...
            className={cn(
              "w-full border-b border-border-subtle px-4 py-3 text-left transition-colors",
              selectedId === note.id ? "bg-accent-subtle" : "hover:bg-bg-hover",
            )}
          >
            <p className={cn("mb-0.5 truncate text-xs font-medium", selectedId === note.id ? "text-accent" : "text-text-primary")}>
              {note.title || "Sans titre"}
            </p>
            <p className="truncate text-[10px] text-text-muted">{excerpt(note.content)}</p>
          </button>
        ))}
      </div>
    </div>
...
export function NoteEditor({ note, onUpdate }: Props) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: { class: "prose prose-invert max-w-none focus:outline-none text-text-primary text-sm" },
    },
    onUpdate({ editor }) {
      setSaveState("unsaved");
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border-subtle px-6 py-3">
        <input
          ...
          className="flex-1 bg-transparent text-sm font-semibold text-text-white focus:outline-none"
        />
        <span className="text-[10px] text-text-muted">
          {saveState === "saving" ? "Enregistrement..." : saveState === "unsaved" ? "Non enregistré" : "Enregistré"}
        </span>
      </div>
...
export function NotesView({ projectId, initialNotes }: { projectId: string; initialNotes: Note[] }) {
...
.prose-invert p { color: var(--color-text-primary); margin: 0.75em 0; }
.prose-invert h1, .prose-invert h2, .prose-invert h3 { color: var(--color-text-white); font-weight: 600; }
.prose-invert code { background: var(--color-bg-elevated); border-radius: 3px; padding: 0.1em 0.3em; font-size: 0.875em; }
.prose-invert pre { background: var(--color-bg-elevated); border: 1px solid var(--color-border-subtle); border-radius: 6px; padding: 1em; }
.prose-invert blockquote { border-left: 3px solid var(--color-border-default); padding-left: 1em; color: var(--color-text-secondary); }
.ProseMirror .is-editor-empty:first-child::before { content: attr(data-placeholder); color: var(--color-text-muted); pointer-events: none; float: left; height: 0; }
```
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  const workspace = await db.select().from(workspaces).where(eq(workspaces.id, workspaceId)).limit(1).then((r) => r[0]);
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border-subtle">
      <table className="w-full">
        <thead>
          <tr className="border-b border-border-subtle bg-bg-elevated">
            <th className="px-5 py-3 text-left text-xs font-medium text-text-muted">Membre</th>
            <th className="px-5 py-3 text-left text-xs font-medium text-text-muted">Rôle</th>
            <th className="px-5 py-3 text-left text-xs font-medium text-text-muted">Rejoint le</th>
            <th className="px-5 py-3" />
          </tr>
        </thead>
        <tbody>
          {members.map((member) => (
            <tr key={member.userId} className="border-b border-border-subtle bg-bg-app">
              <td className="px-5 py-3">
                <div className="flex items-center gap-3">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-accent-subtle text-xs font-bold text-accent">
              ...
              <td className="px-5 py-3">
                <span className="rounded bg-bg-elevated px-2 py-0.5 text-[10px] font-medium text-text-secondary capitalize">
                  {member.role}
                </span>
              </td>
              <td className="px-5 py-3 text-xs text-text-muted">
                {new Date(member.joinedAt).toLocaleDateString("fr-CH", { day: "numeric", month: "short", year: "numeric" })}
              </td>
...
export function InviteModal({ workspaceId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="mb-5 flex items-center justify-between">
          <h2 className="font-semibold text-text-white">Inviter un membre</h2>
          <button onClick={onClose} className="text-text-muted hover:text-text-secondary">
            <X size={16} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-text-secondary">Adresse e-mail</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-text-secondary">Rôle</label>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value as "member" | "admin")}
              className="w-full rounded-md border border-border-default bg-bg-app px-3 py-2 text-sm text-text-primary focus:border-accent focus:outline-none"
            >
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="rounded-md px-4 py-1.5 text-sm text-text-secondary hover:text-text-primary">
              Annuler
            </button>
            <button
              type="submit"
...
type Member = { userId: string; role: string; joinedAt: Date; name: string; email: string; image: string | null };
type Workspace = { id: string; name: string };
...
export function MembersPageClient({
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-text-white">Membres</h1>
        <button
