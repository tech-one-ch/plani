...
prod: ## Start production stack using GHCR image (needs .env.prod)
...
migrate-docker: ## Run migrations inside the running Docker app container
	docker exec $$($(COMPOSE_FULL) ps -q web) node -e \
	  "require('./apps/web/server.js')" 2>/dev/null || \
	  echo "Run 'make dev-docker' first, then retry."
